...
impl From<alkahest_terminal::TerminalProfile> for TerminalProfileInfo {
    fn from(p: alkahest_terminal::TerminalProfile) -> Self {
        Self {
            name: p.name,
...
impl From<alkahest_terminal::TerminalMatch> for TerminalMatchInfo {
    fn from(m: alkahest_terminal::TerminalMatch) -> Self {
        Self {
...
/// Detect the user's default shell via `alkahest-terminal`.
#[tauri::command]
#[allow(clippy::unnecessary_wraps)]
pub fn terminal_detect_shell() -> Result<String, String> {
    Ok(alkahest_terminal::detect_default_shell())
}
...
pub fn terminal_get_profiles() -> Result<Vec<TerminalProfileInfo>, String> {
    Ok(alkahest_terminal::detect_profiles()
        .into_iter()
...
/// buffered text, then delegates to `alkahest_terminal::find_in_terminal`.
#[tauri::command]
...
pub fn terminal_find_in_buffer(
    ...
    let opts = alkahest_terminal::FindOptions {
    ...
    let mut grid = alkahest_terminal::TerminalGrid::new(rows, cols);
    let template = alkahest_terminal::Cell::default();
    ...
    let matches = alkahest_terminal::find_in_terminal(&grid, &query, &opts);
