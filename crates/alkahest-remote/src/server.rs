//! `Alkahest` Server — runs on the remote machine.
//!
...
impl Default for AlkahestServer {
    fn default() -> Self {
        Self::new()
    }
}
...
impl AlkahestServer {
    ...
    pub async fn start_tcp(self: Arc<Self>, port: u16) -> Result<()> {
        ...
        log::info!("Alkahest Server listening on port {port}");
        loop {
...
mod tests {
    ...
    #[test]
    fn base64_roundtrip() {
        let original = b"Hello, Alkahest remote server!";
    ...
    async fn server_fs_mkdir_and_stat() {
        let server = AlkahestServer::new();
        let tmp = std::env::temp_dir().join("alkahest_test_mkdir");
