...
impl SshConnection {
    ...
    pub async fn install_server(&mut self) -> Result<()> {
        ...
        let check = self
            .exec_command("~/.alkahest-server/alkahest-server --version 2>/dev/null || echo missing")
            .await?;

        if check.stdout.trim() == version {
            log::info!("Alkahest Server {version} already installed");
            self.state = ConnectionState::Connected;
            return Ok(());
        }
        ...
        self.exec_command("mkdir -p ~/.alkahest-server").await?;
        log::info!("installing Alkahest Server {version} on remote");
        self.state = ConnectionState::Connected;
        Ok(())
    }
