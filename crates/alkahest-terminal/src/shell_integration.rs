...
if [[ -z "$__alkahest_installed" ]]; then
    __alkahest_installed=1
    trap '__alkahest_preexec "$BASH_COMMAND"' DEBUG
    PROMPT_COMMAND="__alkahest_prompt_command${PROMPT_COMMAND:+;$PROMPT_COMMAND}"
fi
"#;
...
if [[ -z "$__alkahest_installed" ]]; then
    ...
    add-zsh-hook preexec __alkahest_preexec
    add-zsh-hook precmd __alkahest_precmd
fi
"#;

const FISH_INTEGRATION: &str = r#"
function __alkahest_preexec --on-event fish_preexec
    printf '\e]633;C\a'
    printf '\e]633;E;%s\a' "$argv[1]"
end
function __alkahest_postexec --on-event fish_postexec
...
if (-not $__alkahest_installed) {
    $__alkahest_installed = $true
    $__alkahest_original_prompt = $function:prompt
    function prompt {
        $exitCode = $LASTEXITCODE
        [Console]::Write("`e]633;D;$exitCode`a")
        [Console]::Write("`e]633;A`a")
        [Console]::Write("`e]633;P;Cwd=$PWD`a")
        & $__alkahest_original_prompt
    }
