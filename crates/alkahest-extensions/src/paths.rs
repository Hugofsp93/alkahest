//! Standard filesystem paths for `Alkahest` extension storage.
//!
//! Ported from `src-tauri/src/commands/extension_platform.rs`.

use std::path::PathBuf;

/// Root `Alkahest` data directory (`~/.alkahest`).
pub fn alkahest_data_dir() -> PathBuf {
    dirs::home_dir()
        .unwrap_or_else(|| PathBuf::from("."))
        .join(".alkahest")
}

/// User-installed extensions directory (`~/.alkahest/extensions`).
pub fn user_extensions_dir() -> PathBuf {
    let dir = alkahest_data_dir().join("extensions");
...
pub fn resolve_node_runtime() -> Result<NodeRuntime, String> {
    ...
    Err("Node runtime not found. Install Node.js (>=18) or set ALKAHEST_NODE_BINARY.".into())
}
