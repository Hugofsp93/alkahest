//! Type conversions between `alkahest_text` types and `lsp_types`.
//!
//! Both `alkahest_text::Position` and `lsp_types::Position` are zero-based
//! line/character pairs, so the mapping is straightforward.

use alkahest_text::{Position, Range};

/// Converts a `alkahest_text::Position` to an `lsp_types::Position`.
...
/// Converts an `lsp_types::Position` to a `alkahest_text::Position`.
pub fn lsp_to_position(pos: lsp_types::Position) -> Position {
    Position::new(pos.line, pos.character)
}

/// Converts a `alkahest_text::Range` to an `lsp_types::Range`.
...
/// Converts an `lsp_types::Range` to a `alkahest_text::Range`.
