...
const TRUST_FILE_NAME: &str = ".alkahest-trusted-workspaces.json";
...
fn config_dir() -> PathBuf {
    if let Ok(home) = std::env::var("HOME") {
        PathBuf::from(home).join(".config").join("alkahest")
    } else if let Ok(home) = std::env::var("USERPROFILE") {
        PathBuf::from(home).join(".config").join("alkahest")
    } else {
        PathBuf::from(".config").join("alkahest")
    }
}
...
mod tests {
    ...
    fn temp_trust_dir(name: &str) -> PathBuf {
        let tmp = std::env::temp_dir().join(format!("alkahest-trust-{name}"));
