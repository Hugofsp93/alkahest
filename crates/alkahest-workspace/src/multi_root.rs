...
//! `.code-workspace` (or `.alkahest-workspace`) file.
//!
...
/// Parsed `.code-workspace` / `.alkahest-workspace` file.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
...
/// `.alkahest-workspace` configuration.
...
/// Parse a `.code-workspace` or `.alkahest-workspace` JSON file.
...
pub fn is_workspace_file(path: &Path) -> bool {
    ...
    matches!(ext, "code-workspace" | "alkahest-workspace")
}
...
mod tests {
    ...
    #[test]
    fn save_and_reload() {
        let tmp = std::env::temp_dir().join("alkahest-ws-test.code-workspace");
    ...
    fn is_workspace_file_detection() {
        ...
        assert!(is_workspace_file(Path::new("project.alkahest-workspace")));
    ...
    fn save_workspace_file_fn() {
        let tmp = std::env::temp_dir().join("alkahest-save-ws-test.alkahest-workspace");
        let config = WorkspaceConfig {
